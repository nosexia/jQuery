...
add: function( elem, types, handler, data, selector ) {
	...
	if ( !(eventHandle = elemData.handle) ) {				//eventHandle = eleData.handle 指向fn
		eventHandle = elemData.handle = function( e ) {				//事件回调函数中的this指向elem
			// Discard the second event of a jQuery.event.trigger() and
			// when an event is called after a page has unloaded
	...
	while ( t-- ) {
		 ...
		 * 		handers和events['click']指向同一个数组
		 * 		elemData.events
		 *
		 * 		
		 */
		if ( !(handlers = events[ type ]) ) {			//events[ type ]没值，handlers = events[ type ]
